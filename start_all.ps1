...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd c:\hackthon_2\eduguardian; .venv\Scripts\python.exe chatbot/run_a2a_services.py"
Start-Sleep -Seconds 3

Write-Host "[2/3] Starting FastAPI Gateway & LangGraph (:8000)..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd c:\hackthon_2\eduguardian; .venv\Scripts\uvicorn.exe chatbot.backend.api.main:app --host 0.0.0.0 --port 8000"
Start-Sleep -Seconds 3

Write-Host "[3/3] Starting Frontend React UI (:5173)..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd c:\hackthon_2\eduguardian\chatbot\frontend; npm run dev"
