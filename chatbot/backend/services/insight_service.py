...
INSIGHT_CARD = AgentCard(
    ...
    supported_interfaces=[
        ...
        AgentInterface(url="http://localhost:8001/", protocol_binding="HTTP+JSON"),
    ],
    skills=[
        AgentSkill(
...
def create_insight_service_app() -> FastAPI:
    ...
    async def get_card_legacy() -> dict:
        return MessageToDict(INSIGHT_CARD)

    handler = DefaultRequestHandler(
        agent_executor=StudentInsightExecutor(),
        task_store=InMemoryTaskStore(),
        agent_card=INSIGHT_CARD,
    )
    add_a2a_routes_to_fastapi(
        app,
        agent_card_routes=create_agent_card_routes(INSIGHT_CARD),
