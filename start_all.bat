...
start "A2A_Agents" cmd /k "cd /d c:\hackthon_2\eduguardian && .venv\Scripts\python.exe chatbot/run_a2a_services.py"

timeout /t 3 /nobreak >nul

echo [2/3] Starting FastAPI Gateway (:8000)...
start "FastAPI_Gateway" cmd /k "cd /d c:\hackthon_2\eduguardian && .venv\Scripts\uvicorn.exe chatbot.backend.api.main:app --host 0.0.0.0 --port 8000"

timeout /t 3 /nobreak >nul

echo [3/3] Starting Frontend UI (:5173)...
start "Frontend_UI" cmd /k "cd /d c:\hackthon_2\eduguardian\chatbot\frontend && npm.cmd run dev"
